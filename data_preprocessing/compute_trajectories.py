...
df = df.sort_values(by=['stay_id','start_time'])
df['stay_id_shifted_up'] = df['stay_id'].shift(-1)
df['stay_id_shifted_down'] = df['stay_id'].shift(1)
...
np.save(os.path.join(DATA_FOLDER_PATH,"states/state_space.npy"), MDP_dataset_dict["state_space"])
