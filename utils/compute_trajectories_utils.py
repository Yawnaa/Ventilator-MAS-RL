...
def compute_reward(row):
    if row['stay_id'] != row['stay_id_shifted_up']:  # is last timestep in this icu_stay
...
def build_trajectories(df, state_space, action_space):
    ...
    ********************************
    ...
    for col in state_space:
        if toscale[col].dtype == 'object':
            toscale[col] = df[col].astype(float)
    ...
    df['info'] = df.apply(lambda x: tuple([x['subject_id'], x['hadm_id'], x['stay_id']]), axis=1)
    ...
    for k in tqdm(range(len(unique_infos)), ncols=100):
        ...
        episode_rows = df[(df['subject_id'] == i[0]) & (df['hadm_id'] == i[1]) & (df['stay_id'] == i[2])].sort_values(
            'start_time')
        ...
        for row in range(min(18, len(episode_rows))):
            ...
            if row == end_index:
                ...
                reward = INTERMEDIATE_REWARD_SCALING * (-(
