...
def sample_and_hold(input: pd.DataFrame, hold_period: np.array, adjust = 0):
  ...
  '''
  icustayid_list = list(input['stay_id'])
  i_stay_id = input.columns.get_loc('stay_id')
  ...
  curr_stay_id = temp[0, i_stay_id]
  
  mean_sah = np.nanmean(input.iloc[:, 4:n_col], axis = 0)
  # loop through each column
  for i in range(4,n_col):
    for j in range(n_row):
      if curr_stay_id != temp[j, i_stay_id]:
        # start over since we've seen a new stay_id
        last_charttime[j] = parse(temp[j, i_charttime])
        last_value = np.empty(n_col)
        last_value.fill(-float(math.inf))
        curr_stay_id = temp[j, i_stay_id]
      ...
      if j >= 0: 
        time_del = 0
        if (np.isnan(temp[j,i])) and (temp[j, i_stay_id] == curr_stay_id) and (time_del <= hold_period[i-3]):
          # replacement if NaN via SAH
          if last_value[i] == -float(math.inf):
            k = j
            while (k < temp.shape[0] and np.isnan(temp[k,i]) and curr_stay_id == temp[k, i_stay_id]):
              k += 1
            # If entire episode is Nan then replace by mean
            if k == temp.shape[0] or curr_stay_id != temp[k, i_stay_id]:
...
def preprocess_imputation(df: pd.DataFrame,  N: int, k_param: int, weights = None,  col_names_knn = None, col_names_sah = None):
  ...
  # separate data for SAH and KNN imputation, keeping 'subject_id', 'stay_id', 'hadm_id','start_time' columns intact:
  miss_level = df_drop.isnull().sum() / df_drop.shape[0]
  id_part = df_drop[['subject_id', 'stay_id', 'hadm_id','start_time']]
  ...
  if 'deathtime' in df_sah.columns:
  ...
  c_s = N*1000
  knn_removed_cols = df_knn[['subject_id', 'stay_id', 'hadm_id','start_time','gender', 'dischtime']]
  df_knn_dropped_columns = df_knn.drop(labels=['subject_id', 'stay_id', 'hadm_id','start_time','gender', 'dischtime'], axis = 1).columns
  temp = df_knn.drop(labels=['subject_id', 'stay_id', 'hadm_id','start_time','gender', 'dischtime'], axis = 1).to_numpy()
  ...
  final = df_sah.set_index(['subject_id','stay_id', 'hadm_id', 'start_time']).join(other=df_knn.set_index(['subject_id','stay_id', 'hadm_id', 'start_time']))
